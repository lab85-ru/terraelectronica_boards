...
#define BUFFER_OFFSET          ((uint32_t)0x26000)
...
{ 
  ...
  /* GPIO Configuration ------------------------------------------------------*/
 /* GPIOA configuration */
  GPIO_PinAFConfig(GPIOA, GPIO_PinSource0, GPIO_AF_MCO);
...
{ 
  ...
  LTDC_InitStruct.LTDC_HorizontalSync = 2;
  /* Configure vertical synchronization height */
  LTDC_InitStruct.LTDC_VerticalSync = 1;
  /* Configure accumulated horizontal back porch */
  LTDC_InitStruct.LTDC_AccumulatedHBP = 8; 
  /* Configure accumulated vertical back porch */
  LTDC_InitStruct.LTDC_AccumulatedVBP = 10;  
  /* Configure accumulated active width */  
  LTDC_InitStruct.LTDC_AccumulatedActiveW = 406;
  /* Configure accumulated active height */
  LTDC_InitStruct.LTDC_AccumulatedActiveH = 261;
  /* Configure total width */
  LTDC_InitStruct.LTDC_TotalWidth = 408; 
  /* Configure total height */
  LTDC_InitStruct.LTDC_TotalHeigh = 263;
  ...
  */      
  ...
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = (LCD_PIXEL_WIDTH + 70 - 1); 
  LTDC_Layer_InitStruct.LTDC_VarticalStart = 13;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = (LCD_PIXEL_HEIGHT + 13 - 1);
...
{  
  ...
  LTDC_colorkeying_InitStruct.LTDC_ColorKeyGreen = 0xFF00 & RGBValue;
...
/*
 ...
 |  LCD_TFT R2 <->        |  LCD_TFT G2 <-> PA.06 |  LCD_TFT B2 <->            |
 |  LCD_TFT R3 <-> PB.00  |  LCD_TFT G3 <-> PE.11 |  LCD_TFT B3 <-> PD.10      |
 |  LCD_TFT R4 <-> PA.11  |  LCD_TFT G4 <-> PB.10 |  LCD_TFT B4 <-> PE.12      |
 |  LCD_TFT R5 <-> PA.12  |  LCD_TFT G5 <-> PB.11 |  LCD_TFT B5 <-> PA.03      |
 |  LCD_TFT R6 <-> PA.08  |  LCD_TFT G6 <-> PC.07 |  LCD_TFT B6 <-> PB.08      |
 |  LCD_TFT R7 <-> PG.06  |  LCD_TFT G7 <-> PD.03 |  LCD_TFT B7 <-> PB.09      |
 -------------------------------------------------------------------------------
          |  LCD_TFT HSYNC <-> PC.06  | LCDTFT VSYNC <->  PA.04 |
          |  LCD_TFT CLK   <-> PA.14  | LCD_TFT DE   <->  PF.10 |
           -----------------------------------------------------
          |LCD_TFT backlight <-> PA.00| LCD_ENABLE <->          |
           -----------------------------------------------------
*/
 ...
 /* GPIOA configuration */
  ...
  GPIO_PinAFConfig(GPIOA, GPIO_PinSource8, GPIO_AF_LTDC);
 ...
 /* GPIOB configuration */  
  GPIO_PinAFConfig(GPIOB, GPIO_PinSource0, GPIO_AF_LTDC_R3);
