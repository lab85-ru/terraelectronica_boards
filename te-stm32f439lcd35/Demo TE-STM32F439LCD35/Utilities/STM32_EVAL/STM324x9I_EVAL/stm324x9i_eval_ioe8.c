...
/**
  * @brief  Enables the touch screen interrupt.
...
{ 
  ...
  /* return the data */
  return (__IO uint16_t) (IOE_BufferRX[0] | (uint16_t)IOE_BufferRX[1]<< 8);
}
...
static uint16_t IOE_TS_Read_X(void)
{
  ...
  x = I2C_ReadDataBuffer(IOE_REG_TSC_DATA_X);
  x=(x-110)/12;
  if(x <= 0)
    x = 0;
  return (uint16_t)(x); 
...
{
  int32_t y, yr;
  y= I2C_ReadDataBuffer(IOE_REG_TSC_DATA_Y);
  y=(y-110)/16;
  if(y <= 0)
    y = 0;
  return (uint16_t)(y); 
...
/*
  yr= (y * LCD_PIXEL_HEIGHT) >> 12;
  yr = ((yr * LCD_PIXEL_HEIGHT) / 217) - 12;
...
{
  uint32_t z;
  z = I2C_ReadDataBuffer(IOE_REG_TSC_DATA_Z);
