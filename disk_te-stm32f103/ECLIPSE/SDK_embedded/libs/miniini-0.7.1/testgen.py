...
#for multitags:
#-a and -A is used for number of multitags
#
def help():
...
def comment(comment,ini):
    ini.write(";" + comment + "\n")

def header(header, ini):
    ini.write("[" + header + "]\n")

def tag(name, val, ini):
    ini.write(name + " = " + str(val) + "\n")
...
def ini_tag(ini):
    ...
    if(valtype == "bool"):
        val = random_bool()
    tag(random_string(mintaglength, maxtaglength), val, ini)

def ini_array(name, ini):
    ...
    for elem in range(1, length + 1):
        ...
        if(valtype == "bools"):
            val = random_bool()
        tag(name + str(elem), val, ini)
        if(random.randint(0, 100) < commentchance):
            comment(random_string(minstrlength, maxstrlength), ini)

def ini_multitag(ini):
    length = random.randint(mintags, maxtags)
    val = ""
...
def ini_section(ini):
    header(random_string(mintaglength, maxtaglength), ini)
    if valtype[-1] == "s":
        for array in range(0, random.randint(minarrays, maxarrays)):
            ini_array(chars[array], ini)
    elif valtype[-1] == "m":
...
def ini_file(fname):                                       
    ini = open(fname, "w")
    comment("Generated by testgen.py which is a part of MiniINI", ini)
