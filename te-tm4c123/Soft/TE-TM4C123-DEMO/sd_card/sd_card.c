...
main(void)
{
    ...
    // tick.
    //
    ROM_SysTickPeriodSet(ROM_SysCtlClockGet() / 160);
    ROM_SysTickEnable();
    ...
    //
    GrContextInit(&g_sContext, &g_sNOKIA128X160);
    ...
    //
    GrContextFontSet(&g_sContext, g_psFontCmss12);
    GrStringDrawCentered(&g_sContext, "microSD sample", -1,
    ...
    GrStringDrawCentered(&g_sContext, "terminal", -1,
                         GrContextDpyWidthGet(&g_sContext) / 2, 55, false);
    GrStringDrawCentered(&g_sContext, "to UART0.", -1,
                         GrContextDpyWidthGet(&g_sContext) / 2, 70, false);
    GrStringDrawCentered(&g_sContext, "115000,N,8,1", -1,
                         GrContextDpyWidthGet(&g_sContext) / 2, 85, false);
