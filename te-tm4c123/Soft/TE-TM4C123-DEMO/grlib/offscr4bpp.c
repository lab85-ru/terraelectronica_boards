...
// 
// This is part of revision 1.1 of the Tiva Graphics Library.
//
//*****************************************************************************
...
//*****************************************************************************
static uint32_t
GrOffScreen4BPPColorTranslate(void *pvDisplayData, uint32_t ui32Value)
{
...
//*****************************************************************************
static void
GrOffScreen4BPPPixelDraw(void *pvDisplayData, int32_t i32X, int32_t i32Y,
                         uint32_t ui32Value)
{
    uint8_t *pui8Data;
    ...
    //
    i32BytesPerRow = (*(uint16_t *)(pui8Data + 1) + 1) / 2;

    //
    // Get the offset to the byte of the image buffer that contains the pixel
    // in question.
    //
    pui8Data += (i32BytesPerRow * i32Y) + (i32X / 2) + 6 + (16 * 3);
    ...
    //
    *pui8Data = (*pui8Data & ~(0xf << i32X)) | (ui32Value << i32X);
}
...
//*****************************************************************************
static void
GrOffScreen4BPPPixelDrawMultiple(void *pvDisplayData, int32_t i32X,
                                 ...
                                 const uint8_t *pui8Palette)
{
    ...
    // Compute the number of bytes per row in the image buffer.
    //
    i32BytesPerRow = (*(uint16_t *)(pui8Ptr + 1) + 1) / 2;

    //
    // Get the offset to the byte of the image buffer that contains the
    // starting pixel.
    //
    pui8Ptr += (i32BytesPerRow * i32Y) + (i32X / 2) + 6 + (16 * 3);
...
//*****************************************************************************
static void
GrOffScreen4BPPLineDrawH(void *pvDisplayData, int32_t i32X1, int32_t i32X2,
                         int32_t i32Y, uint32_t ui32Value)
{
    ...
    // starting pixel.
    //
    pui8Data += (i32BytesPerRow * i32Y) + (i32X1 / 2) + 6 + (16 * 3);
    ...
    if(i32X1 & 1)
    {
        ...
        //
        *pui8Data = (*pui8Data & 0xf0) | (ui32Value & 0x0f);
        pui8Data++;
        i32X1++;
    }
    ...
    if(i32X1 == i32X2)
    {
        ...
        //
        *pui8Data = (*pui8Data & 0x0f) | (ui32Value & 0xf0);
    }
}
...
//*****************************************************************************
static void
GrOffScreen4BPPLineDrawV(void *pvDisplayData, int32_t i32X, int32_t i32Y1,
                         int32_t i32Y2, uint32_t ui32Value)
{
    ...
    // starting pixel.
    //
    pui8Data += (i32BytesPerRow * i32Y1) + (i32X / 2) + 6 + (16 * 3);
    ...
    if(i32X & 1)
    {
        ...
        while(i32Y1 <= i32Y2)
        {
            ...
            //
            *pui8Data = (*pui8Data & 0xf0) | ui32Value;
...
//*****************************************************************************
void
GrOffScreen4BPPInit(tDisplay *psDisplay, uint8_t *pui8Image, int32_t i32Width,
                    int32_t i32Height)
{
    //
    // Check the arguments.
    ...
    psDisplay->pfnPixelDraw = GrOffScreen4BPPPixelDraw;
    psDisplay->pfnPixelDrawMultiple = GrOffScreen4BPPPixelDrawMultiple;
    psDisplay->pfnLineDrawH = GrOffScreen4BPPLineDrawH;
    psDisplay->pfnLineDrawV = GrOffScreen4BPPLineDrawV;
    psDisplay->pfnRectFill = GrOffScreen4BPPRectFill;
    psDisplay->pfnColorTranslate = GrOffScreen4BPPColorTranslate;
    psDisplay->pfnFlush = GrOffScreen4BPPFlush;

    //
    // Initialize the image buffer.
    //
    pui8Image[0] = IMAGE_FMT_4BPP_UNCOMP;
...
//*****************************************************************************
void
GrOffScreen4BPPPaletteSet(tDisplay *psDisplay, uint32_t *pui32Palette,
                          uint32_t ui32Offset, uint32_t ui32Count)
{
    ...
    ASSERT(psDisplay);
    ASSERT(pui32Palette);
    ASSERT(ui32Offset < 16);
    ASSERT((ui32Offset + ui32Count) <= 16);
