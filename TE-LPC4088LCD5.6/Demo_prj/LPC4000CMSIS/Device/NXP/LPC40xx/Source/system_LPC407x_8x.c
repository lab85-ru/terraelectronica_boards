/**********************************************************************
* $Id: system_LPC407x_8x.c 60270 2012-12-13 12:15:37Z anderslu $    system_LPC407x_8x.c      2012-01-16
*//**
...
#define PLL1_SETUP            1
#define PLL1CFG_Val           0x00000007
#define CCLKSEL_Val           0x00000101
#define USBCLKSEL_Val         0x00000202
#define EMCCLKSEL_Val         0x00000000
#define PCLKSEL_Val           0x00000002
#define SPIFICLKSEL_Val       0x00000102
...
__no_init uint32_t PeripheralClock;
__no_init uint32_t EMCClock;
__no_init uint32_t USBClock;
__no_init uint32_t SPIFIClock;
...
fpu_init();
  ...
  LPC_SC->FLASHCFG  = FLASHCFG_Val|0x03A; /* Set safe Flash Accelerator setting*/
...
#if __CORE_CLK > 100000000
  LPC_SC->PBOOST   |= 0x03;             /* Power Boost control        */
  LPC_SC->FLASHCFG  = (3)<<12|0x03A;
#else
  LPC_SC->PBOOST   |= 0x00;             /* Power Boost control        */
  LPC_SC->FLASHCFG  = ((__CORE_CLK - 1)/20000000)<<12|0x03A;
#endif
  
#endif

  SCB->VTOR  = (uint32_t)__Vectors & 0x3FFFFF80;
