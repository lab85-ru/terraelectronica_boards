...
{  
  ...
  /*Assign  U1D+ and U1D- pin functions*/
  USB_DP_IOCON = 0x1;
  USB_DM_IOCON = 0x1;

  /*Enable Link LED (P1.18)*/
  USB_OTG_LINK_LED_IOCON = 0x21;
  ...
  /* This is also USBPORTSEL register*/
  LPC_USB->StCtrl = 0;
...
void USB_HwReset (void)
{
  ...
  LPC_USB->DevIntPri = USB_DEV_PRIORITY;
  // USB_Configure
  USB_Configure(FALSE);
  // Set EP priority
  ...
  for(uint32_t i = 0; i < ENP_MAX_NUMB; ++i)
  {
    USB_DDCA[i] = NULL;
  }

  for(uint32_t i = 0; i < USB_DMA_DD_MAX_NUMB; ++i)
  {
    DmaFree[i] = &USB_DmaDesc[i];
  }

#if USB_DMA_ID_MAX_NUMB > 0
  for(uint32_t i = 0; i < USB_DMA_ID_MAX_NUMB; ++i)
  {
  ...
  LPC_USB->UDCAH = (uint32_t)&USB_DDCA;
...
uint32_t Save;
  ...
  LPC_USB->I2C_TX = Conn?0x006:0x007;  // Send OTG Control (Clear/Set) register address
  LPC_USB->I2C_TX = 0x201;             // Clear DP_PULLUP bit, send STOP condition
...
void USB_WakeUp (void)
{
  ...
  while((LPC_USB->USBClkSt & ((1<<1) | (1<<4))) != ((1<<1) | (1<<4)));
...
uint32_t Data, Count;
  ...
  if(EP & 1)
  {
    ...
    if(EpCnfg[EP].bDMA_Transfer)
    {
      ...
      {
        // Trigger transfer
        uint32_t EpReg = USB_Cmd(CMD_USB_SEL_EP | EP,0);
        if(0 == (EpReg & 0x60))
        {
    ...
    else
    {
      ...
      while(EpCnfg[EP].AvbBuff)
      {
        ...
        uint32_t Offset = EpCnfg[EP].Offset;
        ...
        if(Count)
        {
          do
          {
            assert(LPC_USB->Ctrl & (1<<1));
            Data = *(EpCnfg[EP].pBuffer+Offset++);
            if (--Count)
            {
              Data |= (uint32_t)(*(EpCnfg[EP].pBuffer+Offset++))<<8;
              if (--Count)
              {
                Data |= (uint32_t)(*(EpCnfg[EP].pBuffer+Offset++))<<16;
                if (--Count)
                {
                  Data |= (uint32_t)(*(EpCnfg[EP].pBuffer+Offset++))<<24;
                  --Count;
                }
              }
            }
  ...
  else
  {
    ...
    else
    {
      ...
      while(EpCnfg[EP].AvbBuff)
      {
        ...
        while (!(LPC_USB->RxPLen & (1<<11)));
        // Get data size
        Count = LPC_USB->RxPLen & 0x3FF;
...
uint32_t Val;
  ...
  if (USBINTS_bit.USB_INT_REQ_HP)
  {
    ...
    while(USBDEVINTST_bit.EP_FAST)
    {
      ...
      if(EpIntr == 0)
      {
        LPC_USB->DevIntClr = bmUSB_FastInterrupt;
      }
    }
#else
    // high priority SOF
...
#if USB_DMA_DD_MAX_NUMB > 0
  if (LPC_SC->USBIntSt & (1<<2))
  {
    uint32_t UsbDmaInt = 0;
    uint32_t Tmp;
