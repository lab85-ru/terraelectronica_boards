...
pUsbStandardInterfaceDescriptor_t pInterface;
  ...
  {
    // deconfigure device
    for (uint32_t i = 0; i < UsbDevCtrl.Interfaces; ++i)
    {
...
{
  USB_IO_Data(CTRL_ENP_IN ,NULL,(uint32_t)-1,NULL);
  USB_IO_Data(CTRL_ENP_OUT,NULL,(uint32_t)-1,NULL);
}
...
pUsbStandardInterfaceDescriptor_t pData;
  ...
  for( pData = (UsbStandardInterfaceDescriptor_t *)pConfiguration;
      (uint8_t*)pData < (uint8_t *)pConfiguration + pConfiguration->wTotalLength;
       pData =(UsbStandardInterfaceDescriptor_t *)((uint8_t *)pData + pData->bLength))
  {
...
{
#if USB_HIGH_SPEED > 0
const uint8_t ** ppCfgArray = (const uint8_t**)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_CFG_ARRAY:USB_FS_CFG_ARRAY)];
#else
const uint8_t ** ppCfgArray = (const uint8_t**)UsbDescArray[USB_FS_CFG_ARRAY];
#endif
...
static uint32_t ep_numbers;
  ...
  if (pEp == NULL)
  {
    ...
    pEpNext = (pUsbStandardEpDescriptor_t)(((uint8_t *)pInterface)+pInterface->bLength);
  }
  else
  {
    pEpNext = (pUsbStandardEpDescriptor_t)(((uint8_t *)pEp) + pEp->bLength);
  }
  // Find Interface
  for(;ep_numbers != 0 && pEpNext->bLength;
       pEpNext = (pUsbStandardEpDescriptor_t)(((uint8_t *)pEpNext) + pEpNext->bLength))
  {
...
UsbCommStatus_t UsbDevStatus(uint16_t Index)
{
  if(Index == 0)
  {
    InData = ((uint32_t)UsbDevCtrl.Feature.Data) & 0xFF;
    USB_IO_Data(CTRL_ENP_IN,(uint8_t *)&InData,2,(void*)USB_StatusHandler);
    return(UsbPass);
  }
  return(UsbFault);
}
...
UsbCommStatus_t UsbGetDescriptorDevice(uint8_t Index)
{
  if(Index == 0)
  {
    USB_IO_Data(CTRL_ENP_IN,
               (uint8_t *)&UsbStandardDeviceDescriptorStr,
...
#if USB_HIGH_SPEED > 0
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(uint8_t **)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_CFG_ARRAY:USB_FS_CFG_ARRAY)] + Index);
#else
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(uint8_t **)UsbDescArray[USB_FS_CFG_ARRAY] + Index);
#endif // USB_HIGH_SPEED > 0
  USB_IO_Data(CTRL_ENP_IN,
             (uint8_t *)pCfg,
...
uint8_t **ppData;
#if USB_HIGH_SPEED > 0
uint8_t * pLangStr = (uint8_t *)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_LANG_STR:USB_FS_LANG_STR)];
#else
uint8_t * pLangStr = (uint8_t *)UsbDescArray[USB_FS_LANG_STR];
#endif // USB_HIGH_SPEED > 0
uint32_t i;
  if(Index == 0)
  {
  ...
  else
  {
    ...
    // Find String index
  #if USB_HIGH_SPEED > 0
    for(ppData = (*((uint8_t***)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_STR_ARRAY:USB_FS_STR_ARRAY)] + LanguagesOffset));
  #else
    for(ppData = (*((uint8_t***)UsbDescArray[USB_FS_STR_ARRAY] + LanguagesOffset));
  #endif // USB_HIGH_SPEED > 0
        ppData ;
        ++ppData)
    {
 ...
 *************************************************************************/
inline static
UsbCommStatus_t UsbGetQualifierDescriptor(uint8_t Index)
{
  USB_IO_Data(CTRL_ENP_IN,
             (uint8_t *)&UsbQualifierDescriptor,
 ...
 *************************************************************************/
inline static
UsbCommStatus_t UsbGetOtherSpeedDescriptorConfiguration(uint8_t Index)
{
pUsbStandardConfigurationDescriptor_t pCfg;
  if (Index >= UsbStandardDeviceDescriptorStr.bNumConfigurations)
  {
  ...
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(uint8_t **)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_OTHER_CFG_ARRAY:USB_FS_OTHER_CFG_ARRAY)] + Index);
  USB_IO_Data(CTRL_ENP_IN,
             (uint8_t *)pCfg,
              USB_T9_Size(pCfg->wTotalLength,UsbEp0SetupPacket.wLength.Word),
             (void*)USB_StatusHandler);
...
void UsbGetConfiguration(void)
{
     ...
     (UsbEp0SetupPacket.wLength.Word == 1))
  {
    ...
    USB_IO_Data(CTRL_ENP_IN,(uint8_t *)&InData,1,(void*)USB_StatusHandler);
  }
  else
  {
    USB_T9_ERROR_REQUEST();
...
void UsbGetInterface (void)
{
  ...
  {
    USB_IO_Data(CTRL_ENP_IN,
               (uint8_t *)&UsbDevCtrl.AlternateSetting[UsbEp0SetupPacket.wIndex.Word],
                1,
               (void*)USB_StatusHandler);
  }
  else
 ...
 *
 *************************************************************************/
const UsbEP_ExtData_t * USB_EP_ExtFind(uint8_t EP)
{
#if USB_HIGH_SPEED > 0
